...
# suites
suites = ["diamiond", "spade", "heart", "club"]
...
# distributing cards among players2_cards
for i in range(0, 52):
    if i < 26:
...
# function to cards of each players2_cards
# '/t' is tabspace character. It will print one tabspace. Similarly '/n' in newline character. It will print a new line
def print_cards():
...
for i in range(0, no_of_cards):
    if i % 2 == 0: #player 1 will choose first
        ...
        # checking if value of both cards matches or not
        if x1 == x2:
        ...
        while True:
            ...
            x2 = getpass.getpass("\nPlayer 2 : Enter value and type ENTER and then enter type if card\n")
            y2 = getpass.getpass("")

            # to check entered correct or not
            correct2 = 0

            # checking if entered cars is in cards of player 1 or not
            for j in player2_cards:
                if j.suite == y2 and j.value == x2:

                    #deleting entered cards
                    del(player2_cards[player2_cards.index(j)])
                    correct2 = correct2 + 1

                    # now number of card is 1 less
                    no_of_cards = no_of_cards - 1
        ...
        while True:
            ...
            #checking if entered card is in cards of playre 1 or not
            for j in player1_cards:
                if j.suite == y1 and j.value == x1:

                    #deleting entered card
                    del(player1_cards[player1_cards(j)])
                    correct1 = correct1 + 1
